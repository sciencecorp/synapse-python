...
def validate_manifest(manifest_path):
    ...
    try:
        ...
        if 'name' not in manifest:
            console.print(f"[bold red]Error:[/bold red] manifest.json is missing required 'name' property")
            return False
            
        return manifest
    except FileNotFoundError:
        console.print(f"[bold red]Error:[/bold red] manifest.json not found in {manifest_path}")
        return False
    except json.JSONDecodeError:
        console.print(f"[bold red]Error:[/bold red] manifest.json is not valid JSON")
        return False
...
def replace_app_name_in_scripts(app_dir, app_name):
    ...
    for script_path in script_paths:
        if os.path.exists(script_path):
            ...
            content = content.replace('SYNAPSE_APP_EXE="synapse-example-app"', f'SYNAPSE_APP_EXE="{app_name}"')
            
            with open(script_path, 'w') as f:
                f.write(content)
    
    # Handle the systemd service file specially - it needs to be renamed
    systemd_template = os.path.join(app_dir, 'ops', 'package', 'systemd', '{{APP_NAME}}.service')
    systemd_target = os.path.join(app_dir, 'ops', 'package', 'systemd', f'{app_name}.service')
...
def package_app(app_dir):
    ...
    if os.path.exists('/.dockerenv'):
        ...
        if not os.path.exists(package_script):
            console.print(f"[bold red]Error:[/bold red] Package script not found at {package_script}")
            return False
        ...
        with Progress(
            ...
            try:
                ...
                console.print(f"[bold red]Error:[/bold red] Failed to package application: {e}")
                return False
    else:
        ...
        if not os.path.exists(script_path):
            ...
            if not os.path.exists(script_path):
                console.print(f"[bold red]Error:[/bold red] Docker build script not found")
                return False
            ...
            if not os.path.exists(ops_dir) or not os.path.exists(os.path.join(ops_dir, 'package', 'package.sh')):
                # Create the ops directory structure
                os.makedirs(os.path.join(app_dir, 'ops', 'package', 'scripts'), exist_ok=True)
                os.makedirs(os.path.join(app_dir, 'ops', 'package', 'systemd'), exist_ok=True)
                ...
                if os.path.exists(template_ops_dir):
                    ...
                    if os.path.exists(package_sh):
                        shutil.copy(package_sh, os.path.join(app_dir, 'ops', 'package', 'package.sh'))
                    ...
                    if os.path.exists(scripts_dir):
                        for script in os.listdir(scripts_dir):
                            ...
                            dst = os.path.join(app_dir, 'ops', 'package', 'scripts', script)
                            shutil.copy(src, dst)
                    ...
                    if os.path.exists(systemd_dir):
                        for file in os.listdir(systemd_dir):
                            ...
                            dst = os.path.join(app_dir, 'ops', 'package', 'systemd', file)
                            shutil.copy(src, dst)
        ...
        with Progress(
            ...
            try:
                ...
                result = subprocess.run(['bash', script_path], check=True, cwd=app_dir, 
            ...
            try:
                ...
                result = subprocess.run(cmd, check=True, cwd=app_dir, 
                                      capture_output=True, text=True)
...
def find_deb_package(app_dir):
    ...
    console.print(f"[bold red]Error:[/bold red] Could not find .deb package in {app_dir}")
    return None
...
def get_device_credentials(ip_address):
    ...
    import getpass
    console.print("[bold blue]Enter login password (input will be hidden):[/bold blue]", end=" ")
    login_password = getpass.getpass("")
    
    console.print("[bold blue]Enter root password for package installation (input will be hidden):[/bold blue]", end=" ")
...
def deploy_package(ip_address, deb_package_path):
    ...
    if not cached_ip or cached_ip != ip_address or not username or not login_password or not root_password:
    ...
    with Progress(
        ...
        deploy_task = progress.add_task(f"[yellow]Deploying to {ip_address}...", total=3)
        ...
        try:
            ...
            try:
                ...
                console.print(f"[bold red]Error connecting to {ip_address}:[/bold red] {str(e)}")
                console.print("[yellow]Please check your username and password.[/yellow]")
                return False
...
def start_app(ip_address, app_name):
    ...
    with Progress(
        ...
        task = progress.add_task(f"[yellow]Starting {app_name} on {ip_address}...", total=1)
        ...
        try:
            ...
            try:
                ...
                if "error" in output.lower() or "failed" in output.lower():
                    ...
                    console.print(f"[bold red]Error:[/bold red] Failed to start application:\n{output}")
                    return False
            ...
            progress.update(task, visible=False)
            console.print(f"[bold red]Error:[/bold red] Failed to start application: {e}")
            return False
        finally:
            # Clean up connections
            try:
...
def build_app(app_dir, app_name):
    ...
    try:
        ...
        console.print(f"[yellow]Docker image {image} not found, building it first...[/yellow]")
        ...
        try:
            ...
            console.print(f"[blue]Running build_docker.sh...[/blue]")
            subprocess.run(['bash', build_docker_script], check=True, cwd=app_dir)
            console.print(f"[green]Successfully built Docker image.[/green]")
        except subprocess.CalledProcessError as e:
            console.print(f"[bold red]Error:[/bold red] Failed to build Docker image: {e}")
            return False
    ...
    console.print("[dim]This may take a few minutes. You'll see output during the build process.[/dim]")
    ...
    vcpkg_cmd = [
        ...
        '-v', f"{os.path.abspath(app_dir)}:/home/workspace",
        image,
        '/bin/bash', '-c', "cd /home/workspace && if [ -f vcpkg.json ]; then echo 'Installing dependencies from vcpkg.json...'; ${VCPKG_ROOT}/vcpkg install --triplet arm64-linux-dynamic-release; fi"
    ]
    ...
    try:
        ...
        console.print(f"[yellow]Warning: Failed to install dependencies. The build might still succeed.[/yellow]")
    ...
    try:
        ...
        console.print("[bold yellow]Warning: Build completed but binary not found in expected locations.[/bold yellow]")
        ...
        if binary_path:
            ...
            console.print(f"[green]Copied binary to: {os.path.join(build_dir, app_name)}[/green]")
            return True
            
        return False
    except subprocess.CalledProcessError as e:
        console.print(f"[bold red]Error:[/bold red] Failed to build application. Check the CMake output above for details.")
        return False
...
def deploy_cmd(args):
    ...
    try:
        ...
        console.print("[yellow]Required module 'paramiko' not found. Attempting to install...[/yellow]")
        try:
            ...
            console.print(f"[bold red]Error:[/bold red] Failed to install paramiko: {e}")
    ...
    if uri:
        ...
        console.print("[yellow]No URI provided. Package created but not deployed.[/yellow]")
...
def start_app_cmd(args):
    ...
    uri = getattr(args, 'uri', None)
    if not uri:
        console.print("[bold red]Error:[/bold red] No URI provided. Cannot start the application.")
        return
...
def add_commands(subparsers):
    ...
    # Deploy command
    deploy_parser = subparsers.add_parser('deploy', help='Deploy an application to a Synapse device')
    deploy_parser.add_argument('app_dir', nargs='?', default='.', help='Path to the application directory')
    deploy_parser.add_argument('--uri', '-u', help='Device IP address to deploy to', type=str)
    deploy_parser.set_defaults(func=deploy_cmd)
    
    # Start app command
    start_app_parser = subparsers.add_parser('start-app', help='Start an application on a Synapse device')
    start_app_parser.add_argument('app_name', help='Name of the application to start')
    start_app_parser.add_argument('--uri', '-u', help='Device IP address to start the app on', type=str)
